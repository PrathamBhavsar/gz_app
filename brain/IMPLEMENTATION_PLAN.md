...
| 12 | WS event dispatch + deep-link routing + bell badge + lint cleanup | DONE |
...
## Phase 12 — WS Event Dispatch + Deep-Link Routing + Bell Badge + Lint Cleanup

> **Status**: DONE (2026-06-08)
> **Deferred from**: Phases 5, 8, 9, 10
> **12d completed** (2026-05-16): All 65 lint infos resolved — `use_null_aware_elements` (21), `withOpacity→withValues` (20), `unnecessary_underscores` (10), `curly_braces` (3), `activeColor→activeThumbColor` (2), `use_super_parameters` (3), `unnecessary_brace_in_string_interps` (1), `unnecessary_string_interpolations` (1). `flutter analyze` reports 0 issues.
> **Completed** (2026-06-08): `routerProvider` now maps `gzapp://bookings/:id`, `gzapp://stores/:slug`, and `gzapp://reset-password?token=...` into app routes during startup/redirect. `gzapp://notifications` now redirects into the player shell and opens the Notification Center overlay instead of pushing the full-screen notifications route. Home, Sessions, and Wallet show unread bell dots backed by generated Riverpod notification state.
> **Completed** (2026-06-08): `MainPage` now connects `PlayerWsService`, handles `notification.new`, routes `session.ended` back to the sessions hub, extends active-session detail state on `session.extended`, refreshes the sessions hub on `session.started`, and updates booking/session runtime state on `booking.checked_in`. Because the richer Phase 5 notifier stack is absent in this workspace revision, Phase 12 now targets generated runtime providers in `lib/features/sessions/presentation/providers/session_runtime_providers.dart`, and the affected session screens consume that state directly.

### Goal
Wire the three remaining cross-cutting pieces that were deferred from earlier phases, and eliminate all remaining lint infos.

### Deliverables

#### 12a. WS Event Dispatch (deferred from Phase 5 / 9)

`PlayerWsService` is built and connected. Wire up the event dispatch in `lib/core/network/player_ws_service.dart` or in a top-level listener in `main.dart`/`app_router.dart`:

| WS Event | Action |
|---|---|
| `session.ended` | If current route matches `/sessions/active/:id` → `context.go(AppRoutes.sessions)` + show SnackBar "Session ended" |
| `session.extended` | `ref.read(activeSessionNotifierProvider(id).notifier).extendTimer(newEndTime)` |
| `notification.new` | `ref.read(notificationsNotifierProvider.notifier).prependNew(event)` (already scaffolded in `NotificationsNotifier`) |
| `session.started` | `ref.read(activityHubProvider.notifier).refresh()` |
| `booking.checked_in` | `ref.read(bookingDetailProvider(id).notifier).refresh()` |

Implementation notes:
- Listen to `playerWsProvider` in a `ProviderObserver` or a top-level `Consumer` widget inside `MainPage` (player shell).
- Use `ref.listen` (not `ref.watch`) to react to stream events without causing rebuilds.
- Pass `GoRouter` context via `router.routerDelegate.navigatorKey.currentContext`.

#### 12b. Deep-Link Path-Param Routing (deferred from Phase 9 / 10)

URI scheme `gzapp://` is already registered in Android/iOS manifests. Add `initialLocation` parsing in `routerProvider`:

```dart
// In routerProvider, add initialExtra handling or use go_router's path-param redirect:
redirect: (context, state) {
  // existing auth redirect ...

  // Handle deep link URIs mapped to go_router paths
  final uri = state.uri;
  if (uri.scheme == 'gzapp') {
    switch (uri.host) {
      case 'bookings':
        final id = uri.pathSegments.firstOrNull;
        if (id != null) return AppRoutes.bookingDetail.replaceAll(':id', id);
      case 'stores':
        final slug = uri.pathSegments.firstOrNull;
        if (slug != null) return AppRoutes.storeDetail.replaceAll(':slug', slug);
      case 'notifications':
        return null; // handled by overlay — fire showNotificationCenter after redirect
      case 'reset-password':
        return '${AppRoutes.resetPassword}?token=${uri.queryParameters['token']}';
    }
  }
  return null;
}
...
#### 12c. Bell Badge — Unread Count Dot (deferred from Phase 8)

`notificationsNotifierProvider` already exposes `NotificationsData.unreadCount`. Add the badge to all bell icons in player tab headers:

- `HomeMobileLayout` header bell → overlay `EmLiveDot` or a `Badge` widget when `unreadCount > 0`
- `SessionsMobileLayout` header bell → same
- `WalletMobileLayout` header bell → same

Use `Badge(count: unreadCount, child: HugeIcon(...))` or a `Stack` with a positioned red dot.

#### 12d. Lint Cleanup — 65 info-level issues

Fix all analyzer infos (0 errors, 0 warnings, 65 infos):

| Category | Count | Fix |
|---|---|---|
| `use_null_aware_elements` | 21 | `if (x != null) 'k': x` → `'k': ?x`; `if (x != null) x` → `?x` in lists |
| `deprecated_member_use` (withOpacity) | 20 | `.withOpacity(n)` → `.withValues(alpha: n)` |
| `unnecessary_underscores` | 10 | `(_, __)` → `(_, _)` in separatorBuilder lambdas |
| `curly_braces_in_flow_control_structures` | 3 | Wrap single-statement `if` bodies in `{}` |
| `deprecated_member_use` (activeColor) | 2 | `activeColor:` → `activeThumbColor:` on Switch |
| `use_super_parameters` | 3 | `super.x` in constructor params instead of `: super(x: x)` |
| `unnecessary_brace_in_string_interps` | 1 | `${x}` → `$x` for simple identifiers |
| `unnecessary_string_interpolations` | 1 | `'${expr.toString()}'` → `expr.toString()` |

### Brain Sync After Phase 12
- Update `brain/IMPLEMENTATION_PLAN.md` — Phase 12 DONE
- No new registry files needed (cross-cutting changes only)
