...
| 12 | WS event dispatch + deep-link routing + bell badge + lint cleanup | TODO |
...
> **Status**: IN PROGRESS — 12b/12c/12d complete; 12a partially complete
...
> **Partially completed** (2026-06-08): `MainPage` now connects `PlayerWsService`, handles `notification.new`, and redirects from `/sessions/active/:id` on `session.ended`. The `session.extended`, `session.started`, and `booking.checked_in` dispatches remain blocked on the missing Phase 5/8/9 notifier classes in this workspace revision.
