...
> **Completed** (2026-06-08): `MainPage` now connects `PlayerWsService`, handles `notification.new`, redirects from `/sessions/active/:id` on `session.ended`, and surfaces fallback SnackBars for the remaining player events in the current codebase. `routerProvider` now maps `gzapp://bookings/:id`, `gzapp://stores/:slug`, `gzapp://notifications`, and `gzapp://reset-password?token=...` into app routes during startup/redirect. Home, Sessions, and Wallet now show unread bell dots backed by shared notification state.
> **Implementation note**: The plan originally referenced Phase 5/8/9 notifier classes (`activityHubProvider`, `activeSessionNotifierProvider`, `bookingDetailProvider`, `notificationsNotifierProvider`) that are not present in this workspace revision. Phase 12 was completed against the current architecture using `notificationFeedProvider` plus top-level WS event handling in `MainPage`.
